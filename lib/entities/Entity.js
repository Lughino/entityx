...
Entity.prototype._internalLoadDetails = Promise.method(function(details) {
  return Promise.resolve(details).bind(this).then(
      ...
      function(details) {
        ...
        if (!!(this._loadDetailsBefore &&
            (typeof this._loadDetailsBefore === "function"))) {
          this._loadDetailsBefore(details);
        }
...
Entity.prototype._updateSimple = function(updateData, errorCode, messageCode) {
  var that = this;
  return new Promise(function(resolve, reject) {
    that._getRepository().update(updateData).then(function() {
      that._isLoad = false;
      resolve();
    }).catch(function(err) {
      return reject(new ErrorX(errorCode, messageCode, err));
    });
  });
};
